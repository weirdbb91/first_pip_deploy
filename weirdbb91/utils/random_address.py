...
spells = ["가", "나", "다", "라", "마", "바", "사", "아", "자", "차", "카", "타", "파", "하"]


def get_address():
    rand = {}
    address_state = init_address[random.randint(0, len(init_address)-1)]
    for state in address_state:
        rand["state"] = state
        rand["city"] = address_state[state][random.randint(0, len(address_state[state])-1)]
        street = ""
        for x in range(random.randint(2, 6)):
            street += spells[random.randint(0, len(spells)-1)]
        rand["street"] = f"{street}로 {random.randint(1, 999)}"
        rand["block"] = f"{random.randint(1, 999)}호"
    return rand
